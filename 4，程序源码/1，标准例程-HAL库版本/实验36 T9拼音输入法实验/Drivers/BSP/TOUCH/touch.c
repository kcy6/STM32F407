...
static void tp_read_xy(uint16_t *x, uint16_t *y)
{
    ...
    {
        xval = tp_read_xoy(0x90);   /* 读取X轴坐标AD值 */
...
uint8_t tp_scan(uint8_t mode)
{
    if (T_PEN == 0)                                                                                     /* 有触摸按下 */
    {
        ...
        {
            tp_dev.x[0] = (signed short)(tp_dev.x[0] - tp_dev.xc) / tp_dev.xfac + lcddev.width / 2;     /* 将X轴物理坐标转换成逻辑坐标（即对应LCD屏幕上面的X坐标值） */
            tp_dev.y[0] = (signed short)(tp_dev.y[0] - tp_dev.yc) / tp_dev.yfac + lcddev.height / 2;    /* 将Y轴物理坐标转换成逻辑坐标（即对应LCD屏幕上面的Y坐标值） */
        }
 ...
 * @param   px: X方向的比例因子（约接近1越好）
 * @param   py: Y方向的比例因子（约接近1越好）
 * @retval  无
 */
...
void tp_adjust(void)
{
    ...
    short s3;                                                                                                   /* 第3个点的坐标差值 */
    short s4;                                                                                                   /* 第4个点的坐标差值 */
    double px;                                                                                                  /* X轴物理坐标比例,用于判定是否校准成功 */
    double py;                                                                                                  /* Y轴物理坐标比例,用于判定是否校准成功 */
    uint16_t outtime = 0;
