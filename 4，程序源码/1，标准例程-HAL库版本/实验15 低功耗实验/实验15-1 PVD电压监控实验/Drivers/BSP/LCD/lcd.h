...
#define LCD_CS_GPIO_AF              GPIO_AF12_FSMC
...
#define LCD_RS_GPIO_AF              GPIO_AF12_FSMC
...
/* FSMC相关参数定义
 * 默认通过FSMC的存储块1来连接LCD，存储块1有4个片选（FSMC_NE1~4）对应FSMC存储块1的4个区域块
 */
#define LCD_FSMC_NEX                4       /* 使用FSMC_NE1连接LCD_CS，范围：1~4 */
#define LCD_FSMC_AX                 6       /* 使能FSMC_A19连接LCD_RS，范围：0~25 */
 ...
 * 正点原子 M144Z-M4最小系统板STM32F407版使用FSMC_NE1连接LCD_CS，FSMC_A19连接LCD_RS，16位数据线，计算方法如下：
 * 首先FSMC_NE1的基地址为: 0x60000000；FSMC_NEx（x=1~4）的基址为: 0x60000000 + (0x4000000 * (x - 1))
 * FSMC_A19对应地址值：2^19 * 2 = 0x100000；FSMC_Ay（y=0~25）的地址为：2^y * 2
 *
 * LCD->LCD_REG，对应LCD_RS = 0（LCD寄存器）；LCD->LCD_RAM，对应LCD_RS = 1（LCD数据）
 * LCD->LCD_RAM的地址为：0x60000000 + 0x100000 = 0x60100000
 * LCD->LCD_REG的地址为：LCD->LCD_RAM之外的任意地址
 * 由于使用了结构体管理LCD_REG和LCD_RAM（LCD_REG在前，LCD_RAM在后，均为16位数据宽度）
 * 因此结构体的基地址（LCD_BASE） = LCD_RAM - sizeof(LCD_REG) = 0x600FFFFE
 *
 * 更加通用的计算公式为（LCD_CS接片选脚FSMC_NEx（x=1~4），（LCD_RS接地址线FSMC_Ay（y=0~25））：
